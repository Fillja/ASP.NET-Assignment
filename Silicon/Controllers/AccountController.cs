...
[Authorize]
public class AccountController(UserManager<UserEntity> userManager, SignInManager<UserEntity> signInManager, UserFactory userFactory, AddressFactory addressFactory, AddressService addressService, UserService userService, CourseService courseService) : Controller
{
    ...
    public async Task<IActionResult> SavedCourses()
    {
        ...
        if (courseListResult.StatusCode == Infrastructure.Models.StatusCode.OK)
            viewModel.AllCourses = (IEnumerable<CourseEntity>)courseListResult.ContentResult!;

        var user = await _userManager.GetUserAsync(User);
        if (user != null)
        {
            viewModel.BasicForm = _userFactory.PopulateBasicForm(user);
            ...
            if (savedListResult.StatusCode == Infrastructure.Models.StatusCode.OK)
                viewModel.SavedCourses = (IEnumerable<SavedCoursesEntity>)savedListResult.ContentResult!;
        }
    ...
    public async Task<IActionResult> DeleteAll(IEnumerable<SavedCoursesEntity> list)
    {

        var user = await _userManager.GetUserAsync(User);
        if (user != null)
        {
            var deleteResult = await _courseService.DeleteAllSavedCourses(user);
